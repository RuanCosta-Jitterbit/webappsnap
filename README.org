...
3. Run the script:
   ...
   This will snap all the pages on [[https://pmmdemo.percona.com][PMM Demo]]. It takes around 45 minutes. The images are saved in =images/=.

   To snap specific pages (known as /dashboards/ in PMM):

   #+begin_src sh
     ./run-percona-pmm.sh --uid=pmm-home,node-memory,mysql-instance-overview
   #+end_src

   To see a list of available pages:

   #+begin_src sh
     ./run-percona-pmm.sh --list
   #+end_src
...
1. Make copies of these template files:
...
- [X] How to inject custom text strings as form values
